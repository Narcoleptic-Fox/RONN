...
pub fn triu_mask_checked(output: &mut [bool], rows: usize, cols: usize, diagonal: isize) -> nnx_core::error::Result<()> {
    if output.len() != rows * cols {
        return Err(EngineError::ShapeMismatch(
            format!("triu_mask: output.len()={} but rows*cols={}", output.len(), rows * cols)
        ));
    }
...
pub fn tril_mask_checked(output: &mut [bool], rows: usize, cols: usize, diagonal: isize) -> nnx_core::error::Result<()> {
    if output.len() != rows * cols {
        return Err(EngineError::ShapeMismatch(
            format!("tril_mask: output.len()={} but rows*cols={}", output.len(), rows * cols)
        ));
    }
...
pub fn triu_f32_checked(data: &mut [f32], rows: usize, cols: usize, diagonal: isize, value: f32) -> nnx_core::error::Result<()> {
    if data.len() != rows * cols {
        return Err(EngineError::ShapeMismatch(
            format!("triu_f32: data.len()={} but rows*cols={}", data.len(), rows * cols)
        ));
    }
...
pub fn tril_f32_checked(data: &mut [f32], rows: usize, cols: usize, diagonal: isize, value: f32) -> nnx_core::error::Result<()> {
    if data.len() != rows * cols {
        return Err(EngineError::ShapeMismatch(
            format!("tril_f32: data.len()={} but rows*cols={}", data.len(), rows * cols)
        ));
    }
...
pub fn masked_fill_f32_checked(input: &[f32], mask: &[bool], fill_value: f32, output: &mut [f32]) -> nnx_core::error::Result<()> {
    if input.len() != mask.len() {
        return Err(EngineError::ShapeMismatch(
            format!("masked_fill: input.len()={} != mask.len()={}", input.len(), mask.len())
        ));
    }
    if input.len() != output.len() {
        return Err(EngineError::ShapeMismatch(
            format!("masked_fill: input.len()={} != output.len()={}", input.len(), output.len())
        ));
    }
...
pub fn masked_fill_f32_inplace_checked(data: &mut [f32], mask: &[bool], fill_value: f32) -> nnx_core::error::Result<()> {
    if data.len() != mask.len() {
        return Err(EngineError::ShapeMismatch(
            format!("masked_fill_inplace: data.len()={} != mask.len()={}", data.len(), mask.len())
        ));
    }
...
pub fn causal_mask_checked(output: &mut [bool], seq_len: usize) -> nnx_core::error::Result<()> {
    if output.len() != seq_len * seq_len {
        return Err(EngineError::ShapeMismatch(
            format!("causal_mask: output.len()={} but seq_len*seq_len={}", output.len(), seq_len * seq_len)
        ));
    }
...
pub fn apply_causal_mask_f32_checked(scores: &mut [f32], seq_len: usize) -> nnx_core::error::Result<()> {
    if scores.len() != seq_len * seq_len {
        return Err(EngineError::ShapeMismatch(
            format!("apply_causal_mask: scores.len()={} but seq_len*seq_len={}", scores.len(), seq_len * seq_len)
        ));
    }
...
mod tests {
    ...
    fn test_triu_mask() {
        ...
        assert_eq!(mask, [true, true, true, false, true, true, false, false, true]);
    }
    ...
    fn test_tril_mask() {
        ...
        assert_eq!(mask, [true, false, false, true, true, false, true, true, true]);
    }
    ...
    fn test_causal_mask() {
        ...
        assert_eq!(mask, [false, true, true, false, false, true, false, false, false]);
    }
