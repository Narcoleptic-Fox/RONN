...
pub fn conv2d_f32_checked(
    ...
    if h + 2 * pad_h < kh || w + 2 * pad_w < kw {
        return Err(EngineError::ShapeMismatch(
            format!("conv2d: kernel ({}x{}) too large for padded input ({}x{})",
                kh, kw, h + 2 * pad_h, w + 2 * pad_w)
        ));
    }
    let h_out = (h + 2 * pad_h - kh) / stride_h + 1;
    let w_out = (w + 2 * pad_w - kw) / stride_w + 1;
    ...
    if input.len() != expected_input {
        return Err(EngineError::ShapeMismatch(
            format!("conv2d: input.len()={} but expected {}", input.len(), expected_input)
        ));
    }
    if weight.len() != expected_weight {
        return Err(EngineError::ShapeMismatch(
            format!("conv2d: weight.len()={} but expected {}", weight.len(), expected_weight)
        ));
    }
    if output.len() != expected_output {
        return Err(EngineError::ShapeMismatch(
            format!("conv2d: output.len()={} but expected {}", output.len(), expected_output)
        ));
    }
    if let Some(b) = bias {
        if b.len() != c_out {
            return Err(EngineError::ShapeMismatch(
                format!("conv2d: bias.len()={} but c_out={}", b.len(), c_out)
            ));
        }
    }
    conv2d_f32(input, weight, bias, output, batch_size, c_in, h, w, c_out, kh, kw, stride_h, stride_w, pad_h, pad_w);
    Ok(())
}
...
mod tests {
    ...
    fn test_conv2d_checked_valid() {
        ...
        let weight = [1.0f32];
        let mut output = [0.0f32; 4];
        let result = conv2d_f32_checked(&input, &weight, None, &mut output, 1, 1, 2, 2, 1, 1, 1, 1, 1, 0, 0);
        assert!(result.is_ok());
    }
    ...
    fn test_conv2d_checked_zero_stride() {
        ...
        let weight = [1.0f32];
        let mut output = [0.0f32; 4];
        let result = conv2d_f32_checked(&input, &weight, None, &mut output, 1, 1, 2, 2, 1, 1, 1, 0, 1, 0, 0);
        assert!(result.is_err());
    }
}
