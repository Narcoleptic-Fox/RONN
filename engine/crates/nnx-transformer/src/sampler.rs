...
impl Default for SamplerConfig {
    fn default() -> Self {
        Self { temperature: 0.7, top_k: 40, top_p: 0.9, repetition_penalty: 1.1 }
    }
}

impl SamplerConfig {
    pub fn greedy() -> Self {
        Self { temperature: 0.0, top_k: 0, top_p: 1.0, repetition_penalty: 1.0 }
    }
}

/// Sample a token from logits.
pub fn sample(logits: &[f32], config: &SamplerConfig, prev_tokens: &[u32], rng_state: &mut u64) -> u32 {
    let n = logits.len();
    ...
    if config.repetition_penalty != 1.0 {
        for &tok in prev_tokens {
            let i = tok as usize;
            if i < n {
                if probs[i] > 0.0 { probs[i] /= config.repetition_penalty; }
                else { probs[i] *= config.repetition_penalty; }
            }
        }
    }
    ...
    // Temperature
    for v in probs.iter_mut() { *v /= config.temperature; }

    // Softmax
    let max = probs.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
    let mut sum = 0.0f32;
    for v in probs.iter_mut() { *v = (*v - max).exp(); sum += *v; }
    ...
    if config.top_k > 0 && config.top_k < n {
        ...
        for i in 0..n { if probs[i] < cutoff { probs[i] = 0.0; } }
    }
    ...
    if config.top_p < 1.0 {
        ...
        for i in 0..n { if !keep.contains(&i) { probs[i] = 0.0; } }
    }

    // Renormalize
    let sum: f32 = probs.iter().sum();
    if sum > 0.0 { for v in probs.iter_mut() { *v /= sum; } }
