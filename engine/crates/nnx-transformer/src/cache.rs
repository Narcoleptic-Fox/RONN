...
impl LayerCache {
    ...
    pub fn store(&mut self, key: &[f32], value: &[f32]) {
        ...
        debug_assert!(self.len < self.max_seq_len, "KV cache full");
    ...
    pub fn truncate(&mut self, n: usize) { self.len = n.min(self.len); }
...
impl KVCache {
    ...
    pub fn layer(&self, i: usize) -> &LayerCache { &self.layers[i] }
    pub fn layer_mut(&mut self, i: usize) -> &mut LayerCache { &mut self.layers[i] }
    pub fn position(&self) -> usize { self.layers.first().map_or(0, |l| l.len()) }

    pub fn clear(&mut self) { for l in &mut self.layers { l.clear(); } }
    pub fn truncate(&mut self, n: usize) { for l in &mut self.layers { l.truncate(n); } }
