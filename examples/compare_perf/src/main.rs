...
fn main() {
    ...
    for (name, shape) in &sizes {
        ...
        let ronn_throughput =
            (total_elements as f64 * iterations as f64) / ronn_time.as_secs_f64() / 1e9;
        ...
        println!(
            "{:15} | RONN: {:>8.1} ns | Vec: {:>8.1} ns | {:.1}x | {:.2} Gelem/s",
    ...
    let matmul_sizes = vec![("64x64", 64), ("256x256", 256), ("512x512", 512)];
    ...
    for (name, n) in &matmul_sizes {
        ...
        println!(
            "{:15} | {:>8.1} µs | {:>6.2} GFLOPS",
            name, per_op_us, gflops
        );
    }
