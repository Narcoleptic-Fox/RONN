...
fn test_pooled_allocator_performance_benefit() -> Result<()> {
    ...
    // Pooled allocator hit rate should be high, showing reuse works
    // (Performance assertions are too flaky for CI)
    assert!(
        pooled_allocator.get_hit_rate() > 0.9,
        "Hit rate should be >90% for repeated same-size allocations"
    );
